...
static VEE_NODISCARD std::vector<const char *> filter_extensions(std::vector<const char *> &available_extensions,
    ...
    std::ranges::sort(available_extensions, [](const char *a, const char *b) { return std::strcmp(a, b) < 0; });
    std::ranges::sort(requested_extensions, [](const char *a, const char *b) { return std::strcmp(a, b) < 0; });
...
namespace Vee {
    Renderer::Renderer(const Platform::Window &window) {
        ...
        VK_CHECK(vkEnumerateInstanceExtensionProperties(nullptr, &num_extensions, available_extensions.data()))
        ...
        std::vector<const char *> enabled_instance_layers = filter_extensions(
            available_layer_names,
            requested_layer_names
        );
        std::vector<const char *> enabled_instance_extensions = filter_extensions(
...
#if defined(VK_KHR_surface)
            if (presentation_family_index == UINT32_MAX
        ...
        std::vector<const char *> requested_property_names = {
            "VK_KHR_swapchain"
        };
        std::vector<const char *> enabled_property_names = filter_extensions(
            available_property_names, requested_property_names);
        ...
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(gpu, surface, &format_count, surface_formats.data()));
        ...
        VK_CHECK(vkCreateGraphicsPipelines(device, pipeline_cache, 1, &pipeline_info, nullptr, &pipeline))
    }
    ...
    void Renderer::Render() {
        ...
        VK_CHECK(vkAcquireNextImageKHR(device, swapchain, 0, acquire_semaphore, VK_NULL_HANDLE, &image_index));
        ...
        vkCmdPushConstants(command_buffer, pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float), &data);
